...
beforeEach(function () {
    ...
    setAppInstance($app, $container);

    $this->user = new GenericUser('users', ['id' => 1, 'email' => 'test@example.com', 'password' => ''], 'id', 'password');
});

afterEach(function () {
    setAppInstance(null, new Container());
});

function setAppInstance(?App $app, Container $container): void
{
...
it('allows auth route guard when authenticated', function () {
    ...
    expect((new AuthRouteGuard())->can($request))->toBeTrue();
});

it('rejects role guard without matching roles', function () {
    $swoole = new SwooleRequest();
    $swoole->server = ['request_method' => 'GET', 'request_uri' => '/'];
    ...
    expect((new RoleGuard('admin'))->can($request))->toBeFalse();
});

it('allows role guard with matching roles from session claims', function () {
    $swoole = new SwooleRequest();
    $swoole->server = ['request_method' => 'GET', 'request_uri' => '/'];
    ...
    $this->auth->login($this->user, 'session', ['roles' => ['admin']]);

    expect((new RoleGuard('admin'))->can($request))->toBeTrue();
});

it('allows scope guard with matching scopes', function () {
    $swoole = new SwooleRequest();
    $swoole->server = ['request_method' => 'GET', 'request_uri' => '/'];
    ...
    $this->auth->login($this->user, 'session', ['scopes' => ['users:read']]);

    expect((new ScopeGuard('users:read'))->can($request))->toBeTrue();
});
